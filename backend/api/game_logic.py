...
def claim_bingo_unified(card, game: Game, is_fake_user: bool = False) -> Tuple[bool, Optional[str], Optional[str]]:
    ...
    try:
        ...
        if is_first_winner:
            # Do NOT set game completed yet: keep game active for the tie window (2s fake / 0.5s real)
            ...
            # Real first: 0.5s window (fast co-winners only); fake first: 2s so real players can claim and share.
            ...
            window_sec = 2 if is_fake_user else 0.5
